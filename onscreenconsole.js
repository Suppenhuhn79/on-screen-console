...
if (/\bconsole=[1-4]\b/i.test(window.location.search))
{
	let _ =
	{
		...
		CSS:
		{
			"": "background-color:#fcfcfc;color:#777;font-family:monospace;font-size:0.8rem;position:fixed;top:0px;left:0px;width:100vw;margin:0px;padding:0px;box-sizing:border-box;z-index:" + Number.MAX_SAFE_INTEGER + ";border:none;border-bottom:0.3rem solid #888;",
			"ul": "list-style:none;margin:0rem;padding:0rem;color:#bbb;",
			"input": "background-color:inherit;color:inherit;font-family:inherit;font-size:inherit;width:100%;border:none;outline:none;",
			".output": "padding:0px;margin:0px;max-height:30vh;overflow-y:scroll;",
			".output>div": "display:inline-block;min-width:100%;",
			".output>div>div:nth-child(1)": "float:left;margin-left:0.3em",
			".output>div>div:nth-child(2)": "margin-left:1.3em;",
			".output>div:not(.input)": "border-bottom:1px solid #eee;",
			".itm": "display:inline-block;white-space:pre;vertical-align:top;margin-right:0.7em;",
			".expandable:hover": "text-decoration:underline;cursor:pointer;",
			"div.warn": "background-color:#fdfdd8;",
			"div.error": "background-color:#fee8e8;",
			".internal,.boolean,.undefined": "color:#00f;",
			".string": "color:#777;",
			".number": "color:#2a2;",
			".object": "color:#3bd;",
			".function": "color:#6af;font-weight:bold;",
			".error": "color:#c44;",
			".objdesc": "color:#bbb;"
		},
		PREFIXES:
	...
	if (_.LOG_LEVEL > 0)
	{
		...
		_.prompt.setAttribute("placeholder", ">>");
		...
		_.escapeHtml = (val) => String(val).replaceAll("<", "&lt;").replaceAll(">", "&gt;");
		_.write = (kind, ...vals) =>
		{
			let content = _.ne("div.lin");
			switch (kind)
			{
			case "input":
				content.innerHTML = _.escapeHtml(vals[0]);
				break;
			case "result":
				content.appendChild(_.formatVal(vals[0], kind));
				break;
			default:
				for (let v of vals[0])
					content.appendChild(_.formatVal(v, kind));
			};
			_.output.appendChild(_.ne("div." + kind, _.ne("div", (_.PREFIXES[kind] ?? "&nbsp;")), content));
			_.refresh();
		...
		_.formatVal = (val, kind) =>
		{
			const __objOut = (obj, ele) =>
			{
				...
				if (obj instanceof Element)
					objDesc = _.escapeHtml(/<.+?>/.exec(obj.outerHTML)[0]);
				if (!!objDesc)
			...
			else
			{
				...
				case "string":
					if (val === "")
						...
						val = "\"" + val.replaceAll("\"", "\\\"") + "\"";
					span.innerHTML = _.escapeHtml(val);
					break;
				case "number":
					span.innerHTML = val;
					break;
				...
				case "object":
					...
					{
						span.classList.remove("object");
						span.appendChild(_.ne("span.error", _.escapeHtml(val), _.ne("br")));
						if (!!val.stack)
		...
		_.refresh = () => (!!document.body) ? document.body.style.marginTop = Math.floor(_.body.getBoundingClientRect().height)+"px": null;
		console.error = (...vals) =>
		{
